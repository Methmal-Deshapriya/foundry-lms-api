...
import { getPublicCoursesService } from "../services/v1/catalog/catalog.service.js";
...
// Attaches directly to a LearningService — see the 2026-09-22 category
// layer removal plan.
const courseData = ({ id, serviceId, slug, price, certificateEnabled }) => ({
  id,
  serviceId,
  slug,
...
const intakeData = ({ id, courseId, serviceId, intakeKey, seasonal = false, status = "DRAFT" }) => ({
  id,
  courseId,
  serviceId,
  intakeKey,
...
async function cleanup() {
  await prisma.enrollment.deleteMany({ where: { course: { serviceId: { in: [ids.paidService, ids.freeService] } } } });
  await prisma.intake.deleteMany({ where: { serviceId: { in: [ids.paidService, ids.freeService] } } });
  await prisma.course.deleteMany({ where: { serviceId: { in: [ids.paidService, ids.freeService] } } });
...
describe.runIf(runDatabaseIntegration)("LearningService database policy boundary", () => {
  beforeAll(async () => {
    ...
    await prisma.course.createMany({ data: [
      courseData({ id: ids.paidCourse, serviceId: ids.paidService, slug: "verify-paid-course", price: 1000, certificateEnabled: true }),
      courseData({ id: ids.freeCourse, serviceId: ids.freeService, slug: "verify-free-course", price: 0, certificateEnabled: false }),
    ] });
  ...
  it("preserves the migrated service/course relation", async () => {
    expect(await prisma.learningService.count()).toBeGreaterThanOrEqual(3);
    const [orphanCount] = await prisma.$queryRaw`SELECT COUNT(*)::int AS count FROM courses WHERE service_id IS NULL`;
  ...
  it("discovers an additional supported service and resolves its public slug without code registration", async () => {
    ...
    const catalog = await getPublicCoursesService("verify-free");
    expect(catalog.serviceId).toBe(ids.freeService);
    expect(catalog.courses).toEqual(expect.arrayContaining([
      expect.objectContaining({ id: ids.freeCourse, slug: "verify-free-course" }),
    ]));
  });

  it("blocks direct identity rewriting and immutable course fields", async () => {
  ...
  it("enforces course pricing policy without the API", async () => {
    await expect(prisma.course.create({ data: courseData({ id: "e1000000-0000-4000-8000-000000000010", serviceId: ids.freeService, slug: "verify-free-bad-price", price: 1, certificateEnabled: false }) })).rejects.toBeTruthy();
    await expect(prisma.course.create({ data: courseData({ id: "e1000000-0000-4000-8000-000000000011", serviceId: ids.paidService, slug: "verify-paid-bad-price", price: 0, certificateEnabled: true }) })).rejects.toBeTruthy();
  });

  it("enforces intake date shape without the API", async () => {
    await expect(prisma.intake.create({ data: intakeData({ id: "e1000000-0000-4000-8000-000000000012", courseId: ids.paidCourse, serviceId: ids.paidService, intakeKey: "VERIFY-PAID-NO-DATES" }) })).rejects.toBeTruthy();
  });

  it("serializes concurrent evergreen inserts", async () => {
    const attempts = await Promise.allSettled([
      prisma.intake.create({ data: intakeData({ id: "e1000000-0000-4000-8000-000000000020", courseId: ids.freeCourse, serviceId: ids.freeService, intakeKey: "VERIFY-FREE-A" }) }),
      prisma.intake.create({ data: intakeData({ id: "e1000000-0000-4000-8000-000000000021", courseId: ids.freeCourse, serviceId: ids.freeService, intakeKey: "VERIFY-FREE-B" }) }),
    ]);
  ...
  it("enforces enrollment source/payment and blocks service archive with active learning", async () => {
    ...
    await prisma.intake.create({ data: intakeData({ id: "e1000000-0000-4000-8000-000000000030", courseId: ids.paidCourse, serviceId: ids.paidService, intakeKey: "VERIFY-PAID-ACTIVE", seasonal: true, status: "OPEN_ACTIVE" }) });
