...
export async function publish(req, res, next) { try { return ApiResponse.send(res, await service.setCoursePublicationService(req.params.id, true, req.user.id)); } catch (error) { next(error); } }
export async function unpublish(req, res, next) { try { return ApiResponse.send(res, await service.setCoursePublicationService(req.params.id, false, req.user.id)); } catch (error) { next(error); } }
