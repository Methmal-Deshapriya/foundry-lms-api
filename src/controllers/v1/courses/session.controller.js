import * as sessionService from "../../../services/v1/courses/session.service.js";
...
export async function createSession(req, res, next) {
  try {
    ...
    const session = await sessionService.createSessionService(courseId, req.body, actorId);
...
export async function reorderSessions(req, res, next) {
  try {
    const { courseId } = req.params;
    const actorId = req.user.id;
    await sessionService.reorderSessionsService(courseId, req.body, actorId);
...
export async function getCourseSessions(req, res, next) {
  try {
    const { courseId } = req.params;
    ...
    let sessions;
    if (isAdmin) {
      sessions = await sessionService.getSessionsAdminService(courseId);
    } else {
      sessions = await sessionService.getSessionsStudentService(courseId, userId);
    }
